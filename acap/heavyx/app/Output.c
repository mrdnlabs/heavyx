...
#define SD_FOLDER "/var/spool/storage/SD_DISK/heavyx"
...
void Output(cJSON* detections, int modelWidth, int modelHeight) {
    ...
    /* HeavyX fix: upstream read group "mode" but main.c writes "model",
     * so the adaptive window always saw 0 and clamped to MAX_ROLLING */
    double averageInferenceTime = ACAP_STATUS_Double("model", "averageTime");
