...
/* HeavyX fix: upstream returns G_SOURCE_REMOVE on transient errors (capture
 * hiccup, settings mid-save), permanently killing detection. Reschedule the
 * idle source after a short delay instead. Never G_SOURCE_CONTINUE from the
 * error paths — an idle source would busy-spin the main loop. */
static gboolean
ImageProcessRestart(gpointer data) {
	(void)data;
...
static gboolean
ImageProcessRetryLater(void) {
	g_timeout_add(2000, ImageProcessRestart, NULL);
	return G_SOURCE_REMOVE;
}
...
ImageProcess(gpointer data) {
	...
	OVERLAY_Update( processedDetections, modelWidth, modelHeight );
	PTZ_Feed( processedDetections, modelWidth, modelHeight );
...
static void HTTP_ENDPOINT_sd_download(const ACAP_HTTP_Response response, const ACAP_HTTP_Request request) {
    (void)request;
    const char* tmp_path = "/tmp/heavyx_export.zip";
    const char* fallback  = "/tmp/heavyx_export.tar.gz";
    ...
    const char* dl_filename  = "heavyx_export.zip";
    ...
    if (!ok) {
        ...
        if (ok) {
            ...
            dl_filename   = "heavyx_export.tar.gz";
        }
    }
